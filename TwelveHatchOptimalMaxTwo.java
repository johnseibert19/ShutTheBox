...
public class TwelveHatchOptimalMaxTwo {
    ...
    public static List<List<Integer>> getPossiblePredecessors(int state) {
        ...
        for (int i = 0; i < posFlips.size(); i++) {
            List<Integer> singleFlip = new ArrayList<>();
            singleFlip.add(posFlips.get(i));
            output.add(singleFlip);

            for (int j = i + 1; j < posFlips.size(); j++) {
                List<Integer> doubleFlip = new ArrayList<>();
                doubleFlip.add(posFlips.get(i));
                doubleFlip.add(posFlips.get(j));
                output.add(doubleFlip);
            }
        }
        ...
        if (cache[state] == null) {
            ...
            for (int i = 2; i <= 12; i++) {
                ...
                for (List<Integer> flips : possiblePredecessors) {
                    if (getSumFromFlipCombination(flips) == i) {
                        int predecessor = getPredecessor(state, flips);
                        val = Math.max(val, cache[predecessor] != null ? cache[predecessor] : throwNewDie(predecessor));
                    }
                }
    ...
    public static void main(String[] args) {
        ...
        try {
            FileWriter writer = new FileWriter("12HatchOptimalMax2.txt");
